...
function(csv2_verify_vendor_snapshot dependency)
  set(snapshot_root
    "${csv2_SOURCE_DIR}/third_party/verification/${dependency}")
  set(file_list
    "${csv2_SOURCE_DIR}/third_party/verification/${dependency}.files")
  set(hash_list
    "${csv2_SOURCE_DIR}/third_party/verification/${dependency}.sha256")
  foreach(required_path IN ITEMS "${snapshot_root}" "${file_list}" "${hash_list}")
    if(NOT EXISTS "${required_path}")
      message(FATAL_ERROR
        "Verification dependency ${dependency} is incomplete: ${required_path}")
    endif()
  endforeach()

  file(STRINGS "${file_list}" expected_files)
  file(STRINGS "${hash_list}" expected_hashes)
  list(LENGTH expected_files expected_count)
  list(LENGTH expected_hashes hash_count)
  if(expected_count EQUAL 0 OR NOT expected_count EQUAL hash_count)
    message(FATAL_ERROR
      "Verification dependency ${dependency} has inconsistent allowlists")
  endif()

  set(hashed_files)
  foreach(hash_line IN LISTS expected_hashes)
    if(NOT hash_line MATCHES "^([0-9a-f]+)  (.+)$")
      message(FATAL_ERROR
        "Verification dependency ${dependency} has a malformed hash entry")
    endif()
    ...
    if(NOT expected_hash_length EQUAL 64)
      message(FATAL_ERROR
        "Verification dependency ${dependency} has a malformed SHA-256")
    endif()
    if(relative_path MATCHES "(^|/)\\.\\.?(/|$)" OR
      ...
      message(FATAL_ERROR
        "Verification dependency ${dependency} has an unsafe path: ${relative_path}")
    endif()
    list(FIND expected_files "${relative_path}" expected_index)
    if(expected_index EQUAL -1)
      message(FATAL_ERROR
        "Verification dependency ${dependency} hash entry is not whitelisted: "
        "${relative_path}")
    endif()
    set(source_path "${snapshot_root}/${relative_path}")
    if(NOT EXISTS "${source_path}" OR IS_DIRECTORY "${source_path}" OR
       IS_SYMLINK "${source_path}")
      message(FATAL_ERROR
        "Verification dependency ${dependency} file is missing or not regular: "
        "${relative_path}")
    endif()
    file(SHA256 "${source_path}" actual_hash)
    if(NOT actual_hash STREQUAL expected_hash)
      message(FATAL_ERROR
        "Verification dependency ${dependency} SHA-256 mismatch for "
        "${relative_path}: expected ${expected_hash}, got ${actual_hash}")
    endif()
    list(APPEND hashed_files "${relative_path}")
  endforeach()

  list(SORT expected_files)
  list(SORT hashed_files)
  if(NOT expected_files STREQUAL hashed_files)
    message(FATAL_ERROR
      "Verification dependency ${dependency} hash allowlist is incomplete")
  endif()
  file(GLOB_RECURSE actual_paths LIST_DIRECTORIES false
    RELATIVE "${snapshot_root}" "${snapshot_root}/*")
  set(actual_files)
  foreach(relative_path IN LISTS actual_paths)
    if(IS_SYMLINK "${snapshot_root}/${relative_path}")
      message(FATAL_ERROR
        "Verification dependency ${dependency} contains a symlink: ${relative_path}")
    endif()
    list(APPEND actual_files "${relative_path}")
  endforeach()
  list(SORT actual_files)
  if(NOT expected_files STREQUAL actual_files)
    message(FATAL_ERROR
      "Verification dependency ${dependency} contains missing or extra files")
  endif()
endfunction()
